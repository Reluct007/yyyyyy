...
const headerInfo = (slug) =>
  products.products.find(
    (product) => slugify(product.title, { lower: true, strict: true }) === slug,
  );
...
function ProductsContent({ params, page = 1 }) {
  ...
  const jsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: `${basic.seo.url}/collection/`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: header.title,
        item: `${basic.seo.url}/collection/${params.slug}/`,
      },
    ],
  };
  ...
  const productArray = allProducts
    .filter((item) => item.category === header.title)
    .map((item) => ({
      image: item.image,
  ...
  const categoryBasePath = `/collection/${params.slug}/`;
  const getPageHref = (pageNumber) =>
    pageNumber === 1 ? categoryBasePath : `${categoryBasePath}page/${pageNumber}/`;
  const startPage = Math.max(
    1,
    Math.min(currentPage - Math.floor(maxPageNumbers / 2), totalPages - maxPageNumbers + 1),
  );
            ...
            {productsPage.map((item, index) => (
              <div key={index} className="h-full rounded-lg border">
                <div className="relative">
                  <Link
                    href={`/product/${item.id || slugify(item.title, { lower: true, strict: true })}/`}
                  >
                    <Image
                      src={item.image}
                      alt={item.title}
