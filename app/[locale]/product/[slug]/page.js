...
export async function generateStaticParams() {
  ...
  for (const locale of locales) {
    for (const item of validProducts) {
      params.push({ locale, slug: slugify(item.title, { lower: true, strict: true }) });
    }
  }
  return params;
}
...
export default async function ProductPage({ params }) {
  ...
  const breadcrumbJsonLd = {
    ...
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": translations.nav?.home || "Home", "item": `${ROOT_URL}/` },
      { "@type": "ListItem", "position": 2, "name": "Collection", "item": `${ROOT_URL}${urlPrefix}/collection/` },
      { "@type": "ListItem", "position": 3, "name": productItem.title, "item": `${ROOT_URL}${urlPrefix}/product/${productId}/` }
    ]
  };
  ...
  const productJsonLd = {
    ...
    "brand": { "@type": "Brand", "name": basic.info.brand },
    "url": `${ROOT_URL}${urlPrefix}/product/${productId}/`
  };
    ...
    <>
      ...
      <section className="py-8 px-2">
        <div className="container mx-auto space-y-8">
          ...
          <ProductGallery mainImage={productItem.image} images={productItem.images || []} title={productItem.title} />
