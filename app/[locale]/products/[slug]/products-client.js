            ...
            {productsPage.map((item, index) => (
              <div key={index} className="rounded-lg border h-full">
                <div className="relative">
                  <Link href={locale === 'en' ? `/product/${item.id || slugify(item.title, { lower: true, strict: true })}` : `/${locale}/product/${item.id || slugify(item.title, { lower: true, strict: true })}`} target="_blank"><Image src={item.image} alt={item.title} className="w-full rounded-t-lg" width={400} height={300} /></Link>
                ...
                <div className="p-4 space-y-2">
                  <Link href={locale === 'en' ? `/product/${item.id || slugify(item.title, { lower: true, strict: true })}` : `/${locale}/product/${item.id || slugify(item.title, { lower: true, strict: true })}`} target="_blank"><h3 className="text-lg font-semibold">{item.title}</h3></Link>
                  ...
                  <Link href={locale === 'en' ? `/product/${item.id || slugify(item.title, { lower: true, strict: true })}` : `/${locale}/product/${item.id || slugify(item.title, { lower: true, strict: true })}`} className="flex items-center gap-2 text-sm text-muted-foreground" target="_blank">
