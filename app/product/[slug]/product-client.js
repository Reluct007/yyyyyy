...
export default function ProductClient({ slug }) {
  ...
  // Get related products from original data
  const allProducts = product.map(p => ({
    ...p,
    id: slugify(p.title, { lower: true, strict: true })
  }));
  const relatedProducts = allProducts.filter(item => item.id !== productItem.id).sort(() => 0.5 - Math.random()).slice(0, 8);
  ...
  const breadcrumbJsonLd = {
    ...
    "itemListElement": [{
      ...
      "position": 2,
      "name": "Product",
      "item": `https://www.labubuwholesale.com/product`
    }, {
      "@type": "ListItem",
      "position": 3,
      "name": productItem.title,
      "item": `https://www.labubuwholesale.com/product/${productItem.id || slugify(productItem.title, { lower: true, strict: true })}`
    }]
  };
  ...
  const productJsonLd = {
    ...
    "image": productItem.image ? `https://www.labubuwholesale.com${productItem.image}` : undefined,
    "brand": {
      "@type": "Brand",
    ...
    "url": `https://www.labubuwholesale.com/product/${productItem.id || slugify(productItem.title, { lower: true, strict: true })}`
  };
            ...
            {relatedProducts.map((item, index) => (
              <div key={index} className="rounded-lg border h-full">
                <div className="relative">
                  <Link href={`/product/${item.id || slugify(item.title, { lower: true, strict: true })}`} target="_blank" aria-label={`View ${item.title} product details`}><Image src={item.image} alt={`${item.title} - Designer collectible product`} className="w-full rounded-t-lg" width={400} height={300} /></Link>
                  <Badge variant="outline" className="absolute left-5 top-5 bg-primary-foreground">
                    <Link href={`/products/${slugify(item.category, { lower: true, strict: true })}`} target="_blank" aria-label={`Browse ${item.category} products`}>{item.category}</Link>
                  </Badge>
                </div>
                <div className="p-4 space-y-2">
                  <Link href={`/product/${item.id || slugify(item.title, { lower: true, strict: true })}`} target="_blank" aria-label={`Learn more about ${item.title}`}><h3 className="text-lg font-semibold">{item.title}</h3></Link>
                  ...
                  <Link href={`/product/${item.id || slugify(item.title, { lower: true, strict: true })}`} className="flex items-center gap-2 text-sm text-muted-foreground" target="_blank" aria-label={`Get details about ${item.title}`}>
