export const basic = {
  // SEO 配置 - 修改这里来更新网站的 TDK
  seo: {
    title: "Poker Kit Wholesale & Custom Poker Sets | PokerKit.com",
    description:
      "B2B supplier of premium poker kits — bulk wholesale, OEM/ODM customization, low MOQs and fast lead times. Ideal for retailers, casinos and promotional gifts. Request samples & pricing today.",
    keywords: [],
    url: "https://pokerset.com",
  },
  info: {
    brand: "Poker Kit",
    link: "https://pokerset.com",
    email: "larry@pokerset.com",
  },
  navbar: {
    brand: "Poker Kit",
    logo: "/logo1.webp",
    buttonText: "Free Quote",
    dialogTitle: "Request a Free Quote",
    dialogDescription: "Please provide any additional details or specific requirements...",
    menu: [
      { label: "Home", href: "/" },
      { label: "Products", href: "/collection/" },
      { label: "About", href: "/about/" },
      { label: "Contact", href: "/contact/" },
    ],
    mobileMenu: [
      { label: "Privacy", href: "/privacy-policy/" },
      { label: "Terms", href: "/terms-of-service/" },
    ],
  },
  cta: {
    title: "Join Our Membership",
    description:
      "Don't miss a thing! Join the Poker Kit community for early access to news, updates, and special deals — be the first to know what's coming next.",
  },
  author: {
    name: "Yves Yu",
    image: "/100x100.png",
    description:
      "I'm a full-stack developer with a passion for creating beautiful and functional websites.",
  },
};
