...
export function SettingsProvider({ children }) {
    ...
    const [productList, setProductList] = useState(DEFAULT_SETTINGS.productList);
    ...
    useEffect(() => {
        try {
            ...
            if (stored) {
                ...
                setProductList(data.productList || DEFAULT_SETTINGS.productList);
            }
        } catch (error) {
    ...
    const saveSettings = async (newSettings) => {
        try {
            ...
            setProductList(newSettings.productList || DEFAULT_SETTINGS.productList);
    ...
    const addProduct = (productData) => {
        ...
        const updatedSettings = {
            ...settings,
            productList: [...(settings.productList || []), newProduct]
        };
    ...
    const updateProduct = (productId, productData) => {
        const updatedList = (settings.productList || []).map(p =>
            p.id === productId
                ? { ...productData, id: productId, updatedAt: Date.now() }
                : p
        );
    ...
    const deleteProduct = (productId) => {
        const updatedList = (settings.productList || []).filter(p => p.id !== productId);
    ...
    const deleteProducts = (productIds) => {
        const updatedList = (settings.productList || []).filter(p => !productIds.includes(p.id));
    ...
    const duplicateProduct = (productId) => {
        const productToDuplicate = (settings.productList || []).find(p => p.id === productId);
        if (!productToDuplicate) return false;
