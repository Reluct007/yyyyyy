...
export function generateProductMetadata(slug, locale = 'en') {
  try {
    ...
    const canonicalUrl = locale === 'en' 
      ? `${ROOT_URL}/product/${productId}/`
      : `${ROOT_URL}/${locale}/product/${productId}/`;

    // 构建描述
    const description = productDescription && productDescription.length > 160 
      ? productDescription.substring(0, 157) + '...' 
      : (productDescription || 'Premium designer collectible from Labubu Wholesale');

    return {
      title: `${productTitle} - Labubu Wholesale`,
