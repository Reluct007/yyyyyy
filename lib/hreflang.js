...
function localizedPath({ logicalPath, locale, defaultLocale }) {
  ...
  return `/${resolvedLocale}${normalizedLogicalPath}`;
}

export function toLocalizedUrl({ siteUrl, logicalPath, locale, defaultLocale = "en" }) {
...
export function buildAlternates({
  ...
  const enabledLocales = Array.isArray(availableLocales) ? availableLocales : allLocales;
